...
# v2.58 关卡目标系统一期：战场编辑器可配置保卫波次/生存计时，胜负分派、HUD 进度、存档迁移与回归同步落地
# v2.59 推进模式骨架：起终点区域、reach 胜利、直接交战连续波、边缘带卷轴相机及编辑器笔刷落地
# v2.60 区域伏击触发器：可视化区域笔刷、敌群/延迟/间隔/冷却/次数配置、进入检测与独立伏击队列落地
# v2.61 关卡编辑器工作区：入口改名，左侧关卡/属性栏与右侧常驻场景画布合并，伏击区域支持画布选中移动，移除建筑/初始炮塔新增入口
# v2.62 多关卡库：新建/复制/重命名/删除/排序/切换、活动试玩关卡、旧单关卡迁移与配置口令 v5 全链路落地
# v2.63 固定面板关卡编辑器：保留 DEBUG 顶栏与页签，关卡列表/画布约束在统一内容区，切页或关闭自动恢复试玩关卡
# v2.64 关卡事件与战役链：顺序动作、场景交互物、Boss 阶段、通关进度/奖励及下一关跳转全链路落地
# v2.88 关卡编辑器三栏工作台：关卡库/自适应画布/标签式属性检查器、悬浮工具轨、动作折叠、搜索及草稿撤销重做落地
# v2.89 地形/物体类型库：分类列表与右侧定义编辑、独立贴图库、可复用类型持久化及关卡通用笔刷/渲染联动落地
# v2.90 关卡任务界面一期：简报/任务贴图/一主两次目标/勋章状态、关卡锁定、预设堡垒选择与开始任务流程落地
# v2.91 关卡出战载具配置：逐关载具白名单、旧关卡全量兼容与预装炮塔合成预览落地
# v2.92 全屏关卡结算：胜负统计、合成载具预览、一主两次目标独立勋章、幂等奖励与关卡流转落地
...
- 原《堡垒编辑器设计.md》（1487 行合集：66 补充块 + 51 详情块 + 基础章节）拆分为 4 份：
  ...
  - 关卡编辑器设计.md：关卡库/场景分层/地形物体/单位实例/事件/任务与进度
...
- 爆炸按事件半径留下程序化焦痕浅坑；未命中非爆炸弹丸的落地坑直径改为弹丸贴图实际显示最长边的 1 倍，无贴图时按几何弹体绘制直径。纯渲染层 decal，120 个 FIFO、45 秒寿命且末 15 秒渐隐；现行关卡设计基准见 `project/docs/关卡编辑器设计.md`。

## v2.65-v2.68：四项连续开发（2026-08-22）
- v2.65 涂装与徽记：堡垒主体/强调色、乘法染色、徽记素材与锚点编辑，编辑器/战场同步呈现。
- v2.66 体验优化：炮塔页 20 秒热管理曲线；模块页 0°/90° 有效放置原点参考。
- v2.67 炮塔预览统一：战场与编辑器复用炮塔核心分层、后座、充能与挂弹绘制。
- v2.68 四向装甲：前后左右受击面、格挡/溢出/穿甲削甲、统一承伤入口、维修分配及 HUD；累计 667 项 sim 回归通过。

## v2.69：防御能量罩（2026-08-22）
- 新增护盾发生器、护盾电容与护盾增效器；模块共用可编辑的装配数量上限，护盾发生器出厂上限为 1；护盾容量/回复动态随模块拆装。
- 承伤顺序统一为护盾→四向装甲→结构，穿甲对护盾无效；破盾后须连续 10 秒未受攻击才恢复回复。
- 回复过程按点耗电、满盾零耗电；加入多边形护罩、受击涟漪、破盾裂片与临时 HUD。
- 新增 15 项护盾/模块上限模拟回归（含旧模块库迁移），累计 682 项。

## v2.70：防守波次自动节奏（2026-08-22）
- 主界面移除手动开波；防守任务按关卡配置自动部署、休整或接踵下一波，其他目标不展示波次概念。
- 防守目标新增波次等待、休整时间（默认 60s）与接踵时间（默认 5s）；两类时间随开关互斥生效，旧关卡自动补默认值。
- 新增 7 项波次节奏模拟回归，累计 689 项。

## v2.71：六边形瓦片护盾表现（2026-08-22）
- 护盾常态改为 128×128 内部场体素材与 64×64 径向柔光素材组合，不显示六边形；受击时命中格先显现、紧邻一圈随后显现，形成一次局部波动后完全隐藏，破盾也不全盾显示六边形。
- 安装发生器后从模块界面返回主战场，护盾从 50% 等比尺寸平滑渐显开展；中段短暂增强边缘辉光，末段轻微越过极限后回弹至 100%。
- 提高内部场体素材可见度；破盾时内部场体立即消失、完整外缘短促增强，再沿整个护盾轮廓分布式抛出大批半透明碎片，六边形仍只承担局部受击波动。
- 内部场体改为同素材双层反向漂移、轻微旋转和错峰明灭；破盾碎片尺寸按护盾短边缩放，并加入个体尺寸差异。
- 护盾主体轮廓由椭圆切回圆角矩形：内部场体、柔光边缘、六边形受击裁剪和破盾碎片出生位置统一服从圆角边界。
- 护盾持续期间，边缘柔光和线宽以可辨识的双频曲线缓慢呼吸；亮度变化强于线宽变化，轮廓位置保持稳定，避免边界抖动。
- 在整圈呼吸之上叠加独立的移动高亮弧段：基础柔光保持均匀，高亮层以更细的柔光、亮线和拖尾约 6.7 秒绕圆角矩形一周，避免重复贴图重叠后抹平局部差异。
- 破盾时内部场体立即消失，增强亮度只保留在外缘；移除船体整矩形受击闪白，溢出伤害改由命中点局部火花反馈，避免战车底盘出现亮色区域。
- 电弧与破盾碎片均为首见事件一次性程序粒子，不增加其他护盾效果贴图。
- 新增 7 项护盾瓦片、受击扩散、边缘覆盖、开展动画、整场破裂、场体流动与碎片缩放模拟回归，累计 696 项。

## v2.72：堡垒素材库分型与统一选择（2026-08-22）
- 素材库新增「堡垒底座」「堡垒主体」「轮胎」三类并注册标准堡垒底座/主体；Track01 转入「轮胎」，原「底座」明确为「炮塔底座」，吉普轮胎与主图不再注册。
- 堡垒编辑器移除底座、主体、履带、轮胎与徽记的上传入口，统一从素材库的内置或已上传条目中选择；旧 dataURL 与资源路径继续兼容。
- 复制堡垒时保留底座、主体、底盘、履带和轮胎配置；新增 1 项分类与引用解析回归，累计 694 项。

## v2.73：轮胎原尺寸与单个/成对单位（2026-08-22）
- 取消轮径编辑与缩放：轮胎本体、落印和编辑器红色参考椭圆统一使用素材原始像素宽高；旧 `r` 字段仅兼容读取且完全忽略。
- WheelDef 新增 `unit: single | pair`；pair 以定义侧 x 为锚，按堡垒中心线展开镜像轮，战场渲染、转向、滚动相位与落印均生成左右两列。
- 新增轮胎单位校验及 2 项成对镜像/落印列回归，累计 696 项。

## v2.74：轮胎按转弯半径旋转（2026-08-22）
- 轮胎设置新增明确的“旋转：是/否”选择；底层沿用 `steered` 兼容旧存档，否时始终与车身平行。
- 选择“是”后，视觉偏角按 `atan(轴距/转弯半径)` 计算：弧线模式优先配置半径，普通轮式底盘按实际 `turnW/vLon` 曲率反推，并受最大前轮转角限制；倒车符号同步。
- 轮胎本体与落印共用同一偏角；新增 2 项半径偏角/直行归零回归，当前累计 701 项。

## v2.75：堡垒战损痕迹（2026-08-22）
- `damageFortress()` 接入主体战损：护盾完全吸收不留痕；装甲格挡生成擦痕，实弹穿透生成弹孔，爆炸伤及结构生成焦痕；命中点逆旋转为主体局部格并投影到车体边缘。
- `fortress.damageMarks[]` 使用确定性尺寸抖动与黄金角旋转，最多保留 60 条并按 FIFO 回收；贴花在主体贴图/徽记之上、炮塔挂点之下绘制。
- 结构值低于 75% 逐步变暗、低于 50% 增加裂纹、低于 25% 冒烟；船体命中增加 0.08 秒白闪和首见一次性金属火花，持续伤害带发射节流。
- 已从两份待开发记录移除完成的 #13；新增 8 项护盾隔离、贴花分类、局部坐标、FIFO、阶段阈值与事件回收回归，累计 709 项。

## v2.76：敌方直线实弹与概率跳弹（2026-08-22）
- 现有行尸、冲核尸、重甲尸、飞蝗、疾行尸统一切换为测试用远程直线实弹；各自配置射程、攻击间隔、弹速、单发伤害和穿深，进入射程停车射击，目标驶出射程后继续追踪。
- 新增独立 `enemyProjectiles` 弹丸池；高速线段进入旋转后的主体贴图 Alpha 才算命中，弹丸会越过底座、履带、轮胎和主体透明区，避免高速穿模与车外提前消失；战场绘制暖红曳光和亮色弹头。
- `damageFortress()` 新增 `penetration` 概率穿深路径：概率为 `clamp(穿深/当前受击面装甲, 5%, 100%)`，穿深达到装甲必穿；失败零结构伤害并生成跳弹浮字、反射短曳光、金属火花和擦痕。护盾仍先于穿深判定。
- 新增 10 项敌人参数、穿深概率、必穿/跳弹、远程发射、高速命中、旋转车体、透明外侧带穿越、主体首像素与随机深入命中回归；旧贴脸伤害测试按弹道飞行时间重校准，累计 719 项。
- 战损覆盖层追加主体贴图 alpha 遮罩：弹孔、焦痕、擦痕、阶段变暗、裂纹与白闪只作用于主体非透明像素，底座、履带和轮子保持独立，不再被主体透明区中的效果覆盖。
- 非爆炸痕迹尺寸与弹丸美术统一：地面弹坑直径 = 弹丸贴图显示最长边 ×1，主体弹孔直径 = ×0.5；程序化弹丸按自身绘制直径回退，爆炸坑与焦痕维持原规则。
- 敌方实弹不再接触主体边缘即命中：进入主体后继续沿 Alpha 不透明跨度深入 18%~70%，每发按事件 ID 稳定随机，最终内部交点驱动装甲面、跳弹与弹孔。

## v2.77：统一单位数据层与目标类别拆分（2026-08-22）
- 新增统一 `UnitDef`、阵营、单位类型、控制器、目标类别与 Boss 扩展协议；现有堡垒、五种敌人和三种友军通过动态适配器映射到稳定单位 ID。
- AI 数据拆为索敌、移动、攻击三个可组合维度；攻城严格限定核心建筑/固定建筑/防御墙，游走作为射程边缘移动规则，可与其他索敌和攻击组合。
- 运行时将旧 `core / building / ally` 目标语义拆为 `fortress / fixedBuilding / combatUnit`，并预留 `coreBuilding`；新生成敌人与友军携带 `unitDefId`，旧命名提供兼容迁移。
- 新增 5 项单位库覆盖、AI 映射、攻城目标约束、旧目标迁移与运行时单位引用回归，累计 724 项。

## v2.78：组合式 AI 运行时决策（2026-08-22）
- 敌人与友军战斗循环接入 `UnitDef.ai`；现有敌人保持堡垒索敌、射程停车和直线实弹，现有友军保持最近敌对索敌、射程停车和即时命中。
- 新增通用射程距离带决策：`rangeEdge` 支持接近、带内横移和过近后撤；`holdRange` 维持旧停车射击语义。
- 新增严格攻城目标选择：只选择可受伤固定建筑和完好防御墙，无合法目标时待命且不回退移动堡垒；敌方实弹池可携带静态目标并结算建筑/墙耐久。
- 独立 `coreBuilding` 待关卡核心获得运行时耐久实体后接入；不复用移动堡垒状态。
- 新增 6 项距离带、兼容保持射程、攻城过滤、最近静态目标、静态实弹命中和无目标待命回归，累计 730 项。

## v2.79：单位编辑器外壳与可持久化单位库（2026-08-22）
- 原“堡垒”页签改名为“单位编辑器”，左侧统一列出堡垒、敌人和友军；堡垒继续复用完整载具专属面板，非堡垒单位进入通用编辑面板。
- 通用面板支持名称、类型、阵营、控制器、生命、速度、体型、飞行属性、目标类别、三段式 AI 以及 Boss 基础倍率。
- 新增版本化 `td-unit-library` 本地存储，支持新建、复制、删除、内置同 ID 覆盖与恢复；敌人和友军运行时优先解析覆盖后的 AI。
- React 检查按函数式 setState、模块级组件、标签可访问性与版本化 localStorage 落地；历史 DebugPanel 直接突变告警未在本轮扩大。
- 新增 4 项自定义单位保存/导出、内置 AI 覆盖、恢复内置和删除回归，累计 734 项；浏览器完成新建→改名→保存→删除闭环且控制台无错误。

## v2.80：单位几何中心统一（2026-08-22）
- 新增共用 `geometryCenter / rectGeometryCenter / centeredRect`；单位原点统一为包围盒宽高除以二，贴图自身中心严格对齐该点。
- 堡垒底座、主体、编辑器预览、坐标换算、默认徽记和新增炮位改用共用中心；偶数尺寸炮位不再被错误推到半格。
- 敌人精灵由脚底锚定改为几何中心锚定，与友军、堡垒和圆形碰撞回退保持同一位置语义。
- 新增 4 项奇偶包围盒、世界中心、贴图中心及堡垒局部原点回归，累计 738 项。

## v2.81：单位视觉、碰撞与远程攻击参数（2026-08-22）
- 单位库存档升级至 v2，新增主体素材、独立显示宽高与统一战斗参数；v1 自定义单位继续兼容加载。
- 单位编辑器新增“单位素材与碰撞”几何中心预览及“攻击参数”面板；素材库新增“单位主体”分类，碰撞半径不再以“体型”混称。
- 现有敌人和友军的生成、移动、射程、攻击周期与伤害优先读取 `UnitDef`；敌方弹丸同步读取弹速、穿深和素材引用，战场主体贴图严格居中。
- 新增 5 项协议版本、尺寸校验、生成生命、敌方弹丸参数/素材和覆盖恢复回归，累计 743 项；类型检查与 React 专项 lint 通过。

## v2.82：稳定单位 ID 接入关卡与生产（2026-08-22）
- 关卡刷敌、区域伏击和 Boss 选择器改为读取统一单位库，运行时按 `unitDefId` 生成自定义敌人并应用其生命、AI、视觉与战斗参数。
- 模块生产配置与编辑器改为引用友方单位稳定 ID，自定义友军能够进入生产队列并保持自身定义。
- 关卡协议升级至 v11、模块库存档升级至 v7；旧 `enemies / boss.kind / produce.kind` 自动迁移为内置稳定单位 ID。
- 新增 5 项生成与迁移回归，累计 748 项。

## v2.83：关卡初始友军与中立单位（2026-08-22）
- 关卡新增 `initialUnits`，使用稳定单位 ID、阵营和几何中心坐标描述开局实体；协议升级至 v12。
- 关卡编辑器可从统一单位库放置、选中、移动、擦除和删除友军/中立单位，并在画布中预览贴图、范围和名称。
- 初始友军接入现有战斗 AI 且不占生产模块上限；中立单位静止、不索敌、不被敌人主动攻击。
- 新增 3 项生成、行为与迁移回归，累计 751 项。

## v2.84：独立核心建筑与攻城索敌（2026-08-22）
- 关卡核心生成独立运行时耐久实体，目标类别为 `coreBuilding`，与移动堡垒彻底分离。
- 核心接入单位阻挡、堡垒碰撞、结构寻路、近战与敌方直线实弹承伤；摧毁后释放占格并判负。
- 攻城 AI 的严格目标集正式完整覆盖核心建筑、固定建筑和防御墙，仍不回退攻击移动堡垒。
- 战场新增核心本体与耐久条，失败界面区分核心摧毁；新增 4 项回归，累计 755 项。

## v2.85：组合式 AI 自爆攻击（2026-08-22）
- 单位战斗协议新增自爆半径与被击毁爆炸档位，单位库存档升级至 v3，并为 v1/v2 自爆草稿补安全默认值。
- 单位编辑器中的自爆参数改为引爆距离、满额伤害、爆炸半径和不爆/半额/满额选择，不再显示无关的弹速、穿深与攻击间隔。
- 抵达合法目标恒满额引爆且不发赏金；被击毁按档位结算并保留正常击杀奖励，使用一次性标志杜绝死亡清理重复爆炸。
- 自爆 AOE 接入堡垒、核心、固定建筑、墙、炮塔、友方/中立单位和可破坏物体；攻城组合严格沿用静态目标集。
- 新增 7 项自爆迁移、抵达、赏金、范围承伤、单次结算与攻城核心回归，累计 762 项。

## v2.86：单位类型专属参数（2026-08-22）
- 单位库存档升级至 v4，新增步兵动画帧率/步幅起伏、飞行器高度/阴影、生物体段、建筑占格/阻挡四套类型配置，并兼容迁移 v1~v3。
- 单位编辑器按类型动态展示专属面板，类型切换重建匹配配置；载具仍复用完整堡垒编辑器。
- 飞行高度、阴影比例、步兵起伏和素材生物多体段接入战场绘制；建筑单位固定原地。
- 建筑矩形占格接入关卡放置、编辑轮廓、地面阻挡、寻路和近战距离，阵亡后释放占格并触发重寻路。
- 新增 6 项迁移、校验、占格、阻挡、静止和死亡释放回归，累计 768 项。

## v2.87：Boss 阶段与单位脚本控制（2026-08-22）
- 单位 Boss 扩展补齐专属血条色、生命阶段动作与击败后动作，普通刷怪生成也会自动应用完整 Boss 配置；单位库存档升级至 v5。
- 关卡动作新增统一单位指令，可按事件来源、稳定单位 ID 或阵营批量选择单位，执行移动、停留、攻击、暂停/恢复/替换 AI 与移除，并支持等待完成。
- 关卡编辑器与单位编辑器都提供动作控件，Boss 阶段、区域事件和交互物共用同一事件队列；关卡协议升级至 v13。
- 完成项 #21 已从两份待开发记录删除；后续装备解锁、目标扩展与剧情演出分别由既有 #7、#22、#18 继续承接。
- 新增 11 项回归，当前累计 779 项。

## v2.93：关卡编辑器收尾与统一规则（2026-08-26）
- 主要/次要目标补“不参与判定、自动完成、按条件判定”三种模式，并让统一条件编辑器支持全部满足/任一满足的多条件组合；空条件不再误判完成。
- 玩家、友方、敌方、中立与中立敌对统一使用一份敌对关系函数，远程弹丸、爆炸、载具炮塔及脚本“最近敌对”均按实例阵营选取目标。
- 任务阶段保持独立任务页；事件不再支持复制/黏贴。关卡链与通关奖励移入可见基础信息区，关卡列表移除“试玩”标记但保留所选关卡预览/试玩逻辑。
- 素材库新增 BGM 分类，关卡 BGM 下拉严格锚定该分类；上传支持 MP3/WAV/OGG，并阻止图片与音频错误互转分类。
- 删除关卡编辑器残留的核心、固定建筑和独立墙体编辑/绘制路径；旧字段进入新版编辑器时清空，现行内容分别由物体、单位或图块表达。
- 正式构建单个 JavaScript 包超过 500 KB 的警告已记录至 `project/docs/待开发记录.md` #29，留待性能优化阶段拆包。

## v2.94：统一音频系统与编辑器接入（2026-08-26）
- 素材库 BGM/SE、IndexedDB 音频正文、原始素材试听统一接入全局音频管理器；提供主音量、BGM、SE 总线与 UI/单位/武器/环境子通道。
- 增加声音预设编辑器：支持多素材权重随机、音量、随机音高、循环、冷却、并发上限、距离衰减和最终效果试听；素材删除会同步清理缓存与音频引用。
- 关卡选择、胜利、失败使用三项全局唯一 BGM；各关卡单独配置战斗 BGM。事件新增“切换 BGM/恢复关卡 BGM”和“播放音效”。
- 单位、载具、炮塔、弹丸、物体提供简化的预设覆盖；运行时已接入移动、炮塔旋转/充能/开火/装填/过热、飞行/命中/穿透/跳弹/爆炸/燃烧、护盾、碰撞/碾压、物体、任务、奖励与 UI 声音。
- 载具碰撞只使用一个全局预设，根据碰撞强度轻微调整音量与音高。音频表现与战斗确定性模拟隔离。
- 已完成内容从待开发 #30 清除；#30 仅保留明确暂缓的音频项目包导出/导入。

## v2.95：统一装备所有权与关卡解锁（2026-08-27）
- 玩家进度新增永久装备所有权、解锁来源记录、已消费勋章与可用勋章计算；关卡通关和未来成就商店购买共用同一解锁入口。
- 关卡编辑器可配置战车、炮塔、模块和徽记解锁奖励；首次胜利发放并在结算界面展示新装备，重复通关不会重复写入。
- 关卡任务界面保留锁定车辆展示但禁止选择，战斗炮塔/模块装配同步执行所有权准入；未登记解锁来源的既有装备继续默认可用。
- #7 已从待开发记录移除；#27 只保留成就商店商品目录、配置和兑换界面。
