...
interface ConfigBundle {
  ...
  level: LevelConfig
  levelLibrary?: LevelLibrary // v5：多关卡库；level 继续作为旧版兼容镜像
  assets?: UploadData[]
...
function buildBundle(): ConfigBundle {
  return {
    ...
    level: LEVEL,
    levelLibrary: levelLibraryForExport(),
...
function validateBundleShape(o: unknown): { ok: true; bundle: ConfigBundle } | { ok: false; error: string } {
  ...
  if (b.levelLibrary !== undefined && (!Array.isArray(b.levelLibrary.levels) || b.levelLibrary.levels.length === 0)) {
    return { ok: false, error: '数据形状不对（levelLibrary.levels 非数组或为空）' }
  }
...
function applyBundle(bundle: ConfigBundle): { ok: true } {
  ...
  if (bundle.moduleDefs !== undefined) {
    ...
    MODULE_DEFS.splice(0, MODULE_DEFS.length, ...modules)
  }
